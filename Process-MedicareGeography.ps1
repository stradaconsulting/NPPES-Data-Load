...
python CleanCSV.py .\data\$npifilename ".\data\tmp_medicareGeography.dat" $filtercol $filterval

Write-Host "Loading Medicare data into SQL Server using bcp..."
bcp dbo.medicare_npi_hcpcs_geography IN .\data\tmp_medicareGeography.dat -f medicare_npi_hcpcs_geography_format.xml -e error_medicareGeography.dat -m 10 -S $server -d $database -U $user -P $pswd #-T Either use -T or -U & -P depending on connection type
